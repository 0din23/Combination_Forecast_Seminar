...
data <- openxlsx::read.xlsx(xlsxFile = PATH_DATA) %>% 
  ...
  mutate(
    "D12" = D12 %>% as.numeric(), #12 month moving average dividends
    "E12" = E12 %>% as.numeric(), #12 month moving average earnings
    "LTY" = LTY %>% as.numeric(), 
    "LTR" = LTR %>% as.numeric(),
    "AAA" = AAA %>% as.numeric(),
    "BAA" = BAA %>% as.numeric(),
...
#  expanding_multi_regression(df = ., label = "Index_excess_forward", m = 40) %>% 
...
data.frame(date = combination_forecast$date, Mean = cumsum(combination_forecast$error1),Median = cumsum(combination_forecast$error2),
  ...
  select(date, Mean, Median, Trimmed_mean, 
         #DMSPE1.0, DMSPE0.9
         ) %>% 
  pivot_longer(data=., cols = colnames(.)[colnames(.)!="date"],
               names_to = "feature", values_to = "Net_SSE") %>% 
